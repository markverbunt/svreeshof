...
$sql = "SELECT * FROM external_events";
$result = $mysqli->query($sql);

$error_event=$_GET['external_event_id'];
$external_event_category=$_GET['external_event_category'];
?>
<!DOCTYPE html>
<html lang="nl-NL">
...
<head>
    ...
    <title>Sv Reeshof Bierteam - Alle Evenementen</title>
    <!-- Favicon-->
                            ...
                            <?php if($is_admin) echo'<ul class="header-dropdown m-r--5">
                                <li class="dropdown">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="/account/add_external_event.php">Voeg een evenement toe</a></li>
                                    </ul>
                                </li>
                            </ul>'; ?>
                        </div>
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        if($external_event_status=="0") echo "<tr class='afgelast'>";
                                            ...
                                            if($external_event_partners=="0") echo "<td><span class='label bg-red'>NEE</span></td>";
                                            else if($external_event_partners=="1") echo "<td><span class='label bg-green'>JA</span></td>";
                                            echo "<td>$external_event_location</td>";
                                            if($external_event_status=="0") echo "<td><span class='label bg-red'>AFGELAST</span></td>";
                                            else if($external_event_status=="1") echo "<td><span class='label bg-green'>ACTIEF</span></td>";
                                            echo "<td><a href='/account/external_event.php?external_event_id=$external_event_id'><i class='material-icons'>visibility</i></a> &nbsp; <a href='/account/edit_external_event.php?external_event_id=$external_event_id'><i class='material-icons'>create</i></a></td>";
                                        echo "</tr>";
                                    }
                                } else {
...
<script>
    function noPlayersFound() {
        swal({
            ...
            window.location.href = '/account/edit_external_event.php?external_event_id=<?php echo $error_event; ?>';
        });
    }
    ...
    function eventSuccessUpdated() {
        swal({
            title: "<?php echo $external_event_category; ?> is succesvol geupdate!",
            text: "Verstuurd eventueel een email notificate van de wijziging",
            type: "success",
            timer: 4000,
            showConfirmButton: false,
        });
